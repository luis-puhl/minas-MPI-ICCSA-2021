...
% Our approach is based on the \arch architecture \cite{Cassales2019aI} and \nd techniques provide by the \minas algorithm \cite{Faria2015minas}. Named \mfog, our distributed algorithm explores load balancing to enable low profile devices at the edge of the net to also work on the classification and detection of unwanted traffic.
...
\cite{Faria2015minas}, by implementation and evaluation of a parallel and
...
devices in IoT scenarios, new constraints apply:
...
% the overall system.
Fog computing infrastructure aims to offload processing from cloud
...
performing the classification step of \minas in an MPI cluster running multiple
...
It is replicated in the fog, and runs on each cluster node, using a configurable number of threads, 
% with one or more nodes and each node multiple processes (limited to the individual CPU core count).

The Novelty Detection Module can be also replicated,
but one instance is intended per local network and one global instance,
...
The design of our distributed \nd architecture includes partitioning the functionalities of the \minas code and establishing the appropriate data flows between different actors.
% The distribution of steps and tasks in various modules opens data distribution and its impacts to discussion.
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    data.
    This choice needs a even more complex a model synchronization mechanism.
  ...
  In Algorithm \ref{alg:MINAS-nd}, at line \ref{alg:MINAS-nd:reclassify}, the
...
identifier is provided as a single character, along with an unique item identifier
(\emph{uid}), which can otherwise be determined from the sample index in the stream.

% muito longo, difícil de entender... - helio
%For output information and format the decision isn't so clear as we can't predict future system integrations needs like only novelty alarms or every samples original \val with assigned label so, we have a compromise and put only enough information for the Sink Module (where the full information from the testing file or stream can be accessed) meaning the format can be defined as a tuple containing \emph{uid} and assigned label.
As its output, the algorithm returns the original sample \val followed by the assigned label. Adjustments can easily be made to provide the output results as a tuple containing \emph{uid} and the assigned label.
...
For evaluation purposes, an \mfog implementation was made using MPI (\emph{Open MPI 4.0.4}). The program is organized in an SPMD programming model, so a single version of the \mfog program was initiated on all nodes, being that one of them would perform the root role, while the others ran as leafs. 
On the root process, a sampler thread is responsible for distributing the sampled flow information (\val) to the classifier nodes, using a round-robin load balancing scheme. The other thread on the root process is responsible for receiving the classification results and for processing the unknown samples in search for novelties. 
Each leaf node runs a model adjustment thread and multiple (up to the number of cores) classifier threads.
