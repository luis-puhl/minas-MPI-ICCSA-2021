...
% dataset ... describe the dataset. The load generator does bla bla bla ... The

Aiming to evaluate our proposal for the effects of distributed novelty detection
in a \iot \nids scenario, we implemented a experimental setup.
Our experimental setup is composed of three Raspberry Pi 3 model B single board
computers connected via Ethernet Switch forming a simple cluster simulating an
\iot network with edge constrained resources.
...
In our setup the dataset is stored in the root's node SD card and is read for
each experiment.
...
From the original dataset, we filtered only samples associated to normal traffic
...
We use two types of evaluation measures for each experiment:
a time measure of the full experiment execution
% extracted by using \emph{GNU Time 1.9} measuring 
and, a set of qualitative measures extracted by a Python script.
...
to extract classification quality measures.
This script takes two inputs, the test dataset and the captured output stream,
...
our evaluation script, each row denotes % one of the datasets original 
...
Lastly, the stream visualization chart shows the summary quality measure
...
Also note that \refminas time in \emph{a} includes the Offline phase while our
...
As for the effects of running a MPI cluster with our implementation we observe
