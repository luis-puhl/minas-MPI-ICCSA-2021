...
\minas attempts to to classify each incoming unlabeled instance according to the
current decision model. Instances not explained by the current model
receive an \textit{unknown} label and are stored in a unknowns-buffer.
...
The algorithm alo has a mechanism to forget clusters that became obsolete and
...
The Novelty Detection function, illustrated in Algorithm \ref{alg:MINAS-nd} groups the
