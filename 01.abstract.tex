...
%   break to 2-3 lines
  ...
  increases, damanding more effort from network administrators and  strategies
  ...
  we address the construction and evaluation of a 
  support mechanism for 
  distributed 
  Network Intrusion Detection Systems (\nids)
  % with 
  based on 
  the MINAS Data Stream Novelty Detection (\nd) algorithm.
...
%   apresentar a estratégia,
  ...
  % The obtained results show a negligible accuracy loss in the distributed version, but also a small reduction in the execution time using low profile devices. Although not efficient, the parallel version showed to be viable as the proposed granularity provides equivalent accuracy and response times. 
